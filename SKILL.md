...
7. Bundled local OCR runs automatically only where the existing text layer is missing, too sparse to explain visible content, or clearly corrupted. A healthy selectable text layer always wins even when a full-page scan sits behind it, and genuinely blank pages are skipped. Rotated scans are normalized before analysis and mapped back to PDF coordinates. For `NEEDS_REVIEW`, inspect `toc_completeness`, `pagination_segments`, `unresolved_toc_rows`, `ocr`, and `review_reasons`. Folder mode writes the review report automatically; after a root-mode review result, rerun with `--report-json` pointing to a task-local work path. Directly inspect only the remaining flagged TOC rows/boundaries. Correct a copy of the report's `links` array, set top-level `reviewed` to `true`, and replay it with `--link-manifest`. Preserve `input_sha256`; source and target page fields are zero-based in manifests.
...
Normal generation does not render text-native pages, launch a browser, or repeat extracted-text analysis. It renders and OCRs only low-text pages with the pinned offline RapidOCR/ONNX runtime. Cached content-stream and canonical visual-resource hashes prove that source artwork is not rewritten. Output destinations are locked during processing, and in folder mode the PDF is published before its hash-bound report commit marker. Read [references/heuristics-and-limitations.md](references/heuristics-and-limitations.md) for scans, ambiguity, manifests, or hidden destinations. Read [references/dependencies.md](references/dependencies.md) for environment details.
